...
<template>
  <div>
    ...
    <GamifyButton @click="onOpenDelete(true)">トレーナーをやめて家に帰る</GamifyButton>
    <h1>てもちポケモン</h1>
    ...
    <GamifyItem><RouterLink to="/trainer">トレーナーを交代する</RouterLink></GamifyItem>
    <GamifyDialog v-if="deleteDialog" id="confirm-delete" title="Check !!!" description="ほんとうにトレーナーをやめるんだな！？" @close="onCloseDelete">
    ...
    <GamifyDialog v-if="nicknameDialog" id="confirm-nickname" title="ニックネーム" :description="`${nicknameDialog.name}ニックネームは？`" @close="onCloseNickname">
      <div class="item">
    ...
    <GamifyDialog v-if="releaseDialog" id="confirm-release" title="check" :description="`${releaseDialog.nickname || releaseDialog.name}`" @close="onCloseRelease">
      <GamifyList :border="false" direction="horizon">
        <GamifyItem>
